...
# One entry per standalone calculator page:
# url slug -> template + standard metadata.
CALC_PAGES = {
    "ais-003": {
...
@app.route("/")
def index():
    # index.html expects a list of standards
    # with fields such as code, title, ready and ev.
    #
    # Convert the dictionary used by the standards pages
    # into the simpler list expected by the existing dashboard.
    dashboard_standards = []
...
@app.route("/standards")
def standards():
    """Display the standards overview page."""
    return render_template(
        "standards.html",
        standards=STANDARDS,
    )
...
def standard_detail(code):
    """Display details for one standard."""
...
def ais003():
    ...
    if x > 1:
        return jsonify({
            "error": "Invalid input: sine expression must be ≤ 1."
        }), 400
