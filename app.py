...
@app.route('/')
def index(): return render_template('index.html', standards=STANDARDS)
@app.route('/api/standards')
def standards(): return jsonify(STANDARDS)

@app.route('/api/calc/ais003', methods=['POST'])
def ais003():
 d=request.get_json(); q,wt,wr=map(float,(d['q'],d['wt'],d['wr']))
 x=math.sin(math.radians(q))*wt/wr
 if x>1: return jsonify({'error':'Invalid input: sine expression must be ≤ 1.'}),400
 grade=100*math.tan(math.asin(x))
 to,tn,gro,grn,gvwo,gvwn,tro,trn,go=map(float,(d['to'],d['tn'],d['gro'],d['grn'],d['gvwo'],d['gvwn'],d['tro'],d['trn'],d['go']))
...
def ais038():
 d=request.get_json(); u,v1,vp,v2=map(float,(d['u'],d['v1'],d['vp'],d['v2']))
 minimum=u*500; ri=((max(v1,vp)-v2)/v2)*500
 return jsonify({'minimum_ohm':minimum,'resistance_ohm':ri,'pass':ri>=minimum})

@app.route('/api/calc/ais039', methods=['POST'])
def ais039():
 d=request.get_json(); ct,t,r=map(float,(d['ct'],d['t'],d['r']))
 corrected=ct+(ct*r*(27-t)/100)
 e,distance=float(d['energy']),float(d['distance']); consumption=e/distance if distance else None
 declared=d.get('declared'); limit=None if declared in (None,'') else float(declared)*1.04
 return jsonify({'corrected_capacity':corrected,'energy_consumption':consumption,'four_percent_limit':limit,'within_limit':None if limit is None or consumption is None else consumption<=limit})

@app.route('/api/calc/ais048', methods=['POST'])
def ais048():
 d=request.get_json(); cap,cr=map(float,(d['capacity'],d['crate']))
 return jsonify({'current':cap*cr,'hours':1/cr,'minutes':60/cr})

@app.route('/api/calc/ais138', methods=['POST'])
def ais138():
 d=request.get_json(); current=float(d['current']); low=d.get('range','low')=='low'
 duty=current/0.6 if low else current/2.5+64
 available=duty*0.6 if low else (duty-64)*2.5
 vdc,r,rf=map(float,(d['vdc'],d['r'],d['rf']))
 body=vdc*(r+rf)/(r*rf); leakage=vdc/(r+2*rf)
 return jsonify({'duty_cycle':duty,'available_current':available,'body_current':body,'earth_leakage':leakage})
